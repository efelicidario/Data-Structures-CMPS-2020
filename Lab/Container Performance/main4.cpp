...
int main()
  ...
  std::cout << "double linked list (back insert)" << std::endl;
  start = clock();
  DLinkedList dll_temp2;
  for ( int loop2=0; loop2< MAX_VALUES; loop2++)
    {
    dll_temp2.InsertBack(sample_data[loop2]);
    }
  med = clock();

  DListNode* dll_ptr = dll_temp2.Search(search_value) ;
  end = clock();
