...
int main()
  ...
  std::cout << "single linked list (back insert)" << std::endl;
  SLinkedList sll_temp2;
  for ( int loop2=0; loop2< MAX_VALUES; loop2++)
    {
    sll_temp2.InsertBack(sample_data[loop2]);
    }
  med = clock();

  SListNode * sll_ptr = sll_temp2.Search(search_value);
  end = clock();
