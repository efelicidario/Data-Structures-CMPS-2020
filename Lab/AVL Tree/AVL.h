...
class AVL
{
    ...
    TreeNode *insertSubtree(TreeNode * subtree, T elem)
    {
        ...
        else if(elem > subtree->data)
        {
            if (subtree->right == nullptr)
                ...
                subtree->right = insertSubtree(subtree->right, elem);
        }
        ...
        else if(elem < subtree->data)
        {
            if(subtree->left == nullptr)
                ...
                subtree->left = insertSubtree(subtree->left, elem);
        }
        int left_height = height(subtree->left);
        int right_height = height(subtree->right);
        if ((left_height > right_height+1) && (subtree->left->data > elem))
        {
            cout << "TestLL" << endl;
            subtree = rightRotate(subtree);
        }
        else if ((left_height > right_height+1) && (subtree->left->data < elem))
        {
            cout << "TestLR" << endl;
            subtree->left = leftRotate(subtree->left);
            subtree = rightRotate(subtree);
        }
        else if ((right_height > left_height+1) && (subtree->right->data < elem))
        {
            cout << "TestRR" << endl;
        ...
        LOG(INFO) << "Returning: " << subtree << endl;
        return subtree;
    }
    ...
    // private helper function for search. Performs the AVL search on the given subtree
