...
// --------------------------------- additions for cmps2020
// there is a function at the bottom of this file InitialzeLogger that I added
// if you want to use a more updated verson of this library just copy
// that part to the new .h file and hopefully it should work
...
// --------------------------------- start additions for cmps2020
// if you want to use a more updated verson of this library just copy
// this part to the new .h file and hopefully it should work
/*
shortcut function  
the easylogging++ library is bit pesky for what we want
so lets wrap up a few function calls to make it easier

set the filename
set it so that LOG(INFO) doesnt display to the console
set a maximum size for the file incase you do an infinite loop
*/

inline void InitializeLogger(const char* name)
{
#ifdef LOGGING_ON  
  // set logname
  el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Filename,name);
  // set a max logfilesize.. so if you log in an infinite loop it wont get 
  // ludacrisly huge and use up all a users file space quota
  // 5,242,880 is 5 Megabytes
  el::Loggers::reconfigureAllLoggers(el::ConfigurationType::MaxLogFileSize,"5242880");
  // dont push LOG(INFO) to console, only to the logfile
  el::Loggers::reconfigureAllLoggers(el::Level::Info,el::ConfigurationType::ToStandardOutput,"false");

#endif
}

//------------------------------------ end additions for cmps2020
